<template>
  <ClientOnly>
    <UDropdownMenu v-if="user" :items="items"
      :ui="{ content: 'divide-gray-200 dark:divide-gray-700 ring-1 ring-gray-200 dark:ring-gray-700 rounded-md shadow-lg', item: 'data-disabled:cursor-text select-text hover:rounded-md hover:bg-gray-100 hover:dark:bg-gray-800' }"
      ...
      <UButton
        ...
        <UChip inset>
          <UAvatar :ui="{ root: 'ring-2 ring-gray-200 dark:ring-gray-500 bg-neutral-100 dark:bg-neutral-700' }"
            size="lg" :alt="`${user.firstname} ${user.lastname}`" :src="user.avatar" />
        </UChip>
      </UButton>
      ...
      <template #item="{ item }">
        ...
        <UIcon v-if="'icon' in item && item.icon" :name="item.icon"
