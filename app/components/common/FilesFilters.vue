  ...
  <UPopover>
    ...
    <template #content>
      <div class="fixed bottom-[5rem] left-1/2 z-50 flex -translate-x-1/2 flex-col items-center gap-4 rounded-xl border border-gray-200 bg-white p-3 shadow-xl backdrop-blur sm:flex-row dark:border-gray-800 dark:bg-gray-900">
        ...
        <USelect v-model="filters.folder" :items="foldersOptions" placeholder="Folder" class="w-32" @update:model-value="updateFilters" />
...
const filters = reactive({
  ...
  folder: typeof route.query.folder === 'string' && route.query.folder !== '' ? Number(route.query.folder) : undefined
})
...
const updateFilters = () => {
  ...
  let query: Record<string, string | number | string[] | undefined | null> = {
    ...
    directory_id: filters.folder || undefined,
